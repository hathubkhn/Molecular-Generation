...
class MolecularGraphNeuralNetwork(nn.Module):
    ...
    def infer_regressor(self, data_batch, train, scale_val=2):
        inputs = data_batch[:-1]
        ...
        with torch.no_grad():
            ...
            predicted_values = self.mlp(molecular_vectors)
        predicted_values = predicted_values.to('cpu').data.numpy()
        predicted_values = np.concatenate(predicted_values) * scale_val
...
class Tester(object):
    ...
    def test_regressor_(self, dataset):
        N = len(dataset)
    ...
    def test_regressor(self, dataset):
        ...
        for i in range(0, N, batch_test):
            ...
            all_correct.extend(correct_values)
        ...
        # Compute metrics
        MAE = SAE / N
        RMSE = np.sqrt(mean_squared_error(all_correct, all_predicted))
        R2 = r2_score(all_correct, all_predicted)
...
if __name__ == "__main__":
    ...
    if task == 'regression':
        result = 'Epoch\tTime(sec)\tLoss_train\tMAE_dev\tMAE_test\tRMSE_test\tR2_test'
    ...
    for epoch in range(iteration):
        ...
        if task == 'regression':
            MAE_dev, RMSE_dev, R2_dev  = tester.test_regressor(dataset_dev)
            MAE_test, RMSE_test, R2_test = tester.test_regressor(dataset_test)
        ...
        result = '\t'.join(map(str, [epoch, time, loss_train,
                                     MAE_dev, MAE_test, RMSE_test, R2_test]))

        if MAE_test < bestMAE:
            bestMAE = MAE_test
            torch.save(model.state_dict(), f"../ckpt/{dataset}_17_4.pth")
