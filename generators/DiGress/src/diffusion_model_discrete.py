...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def on_train_epoch_end(self) -> None:
        ...
        print(f"Epoch {self.current_epoch}: X_CE: {to_log['train_epoch/x_CE'] :.3f}"
                      f" -- E_CE: {to_log['train_epoch/E_CE'] :.3f} --"
                      f" y_CE: {to_log['train_epoch/y_CE'] :.3f}"
                      f" -- {time.time() - self.start_epoch_time:.1f}s ")
        epoch_at_metrics, epoch_bond_metrics = self.train_metrics.log_epoch_metrics()
        print(f"Epoch {self.current_epoch}: {epoch_at_metrics} -- {epoch_bond_metrics}")
    ...
    def on_test_epoch_end(self) -> None:
        ...
        id = 0
        while samples_left_to_generate > 0:
            print(f'Samples left to generate: {samples_left_to_generate}/{self.cfg.general.final_model_samples_to_generate}', end='', flush=True)
            bs = 2 * self.cfg.train.batch_size
            to_generate = min(samples_left_to_generate, bs)
            to_save = min(samples_left_to_save, bs)
            chains_save = min(chains_left_to_save, bs)
        ...
        all_smiles = self.sampling_metrics(samples, self.name, self.current_epoch, self.val_counter, test=True, local_rank=self.local_rank)
        print("Done testing.")
        return all_smiles
    ...
    def get_smiles(self, final_model_samples_to_generate, gen_batchsize=1024):

        self.on_test_epoch_start()
        
        samples_left_to_generate = final_model_samples_to_generate
        samples_left_to_save = final_model_samples_to_generate
        chains_left_to_save = final_model_samples_to_generate

        samples = []
        id = 0
        while samples_left_to_generate > 0:
            print(f'Samples left to generate: {samples_left_to_generate}/{final_model_samples_to_generate}', end='\n', flush=True)
            bs = gen_batchsize
            to_generate = min(samples_left_to_generate, bs)
            to_save = min(samples_left_to_save, bs)
            chains_save = min(chains_left_to_save, bs)
...
F
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
        ...
        """
        print("Generating with batch_size:", batch_size)
