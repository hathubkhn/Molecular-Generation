...
class SamplingMolecularMetrics(nn.Module):
    def __init__(self, dataset_infos, train_smiles):
        ...
        if isinstance(di.n_nodes, np.ndarray):
            di.n_nodes = torch.from_numpy(di.n_nodes)
        ...
        if isinstance(di.node_types, np.ndarray):
            di.node_types = torch.from_numpy(di.node_types)
        ...
        if isinstance(di.edge_types, np.ndarray):
            di.edge_types = torch.from_numpy(di.edge_types)
        ...
        if isinstance(di.valency_distribution, np.ndarray):
            di.valency_distribution = torch.from_numpy(di.valency_distribution)
